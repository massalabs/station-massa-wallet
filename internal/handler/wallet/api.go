...
func AppendEndpoints(api *operations.MassaWalletAPI, prompterApp prompt.WalletPrompterInterface, massaClient network.NodeFetcherInterface, accountsStore *assets.AccountsStore, gc gcache.Cache) {
	...
	api.AddAssetHandler = NewAddAsset(accountsStore)
	api.GetAllAssetsHandler = NewGetAllAssets(accountsStore)
}
