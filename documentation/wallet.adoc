...
Understanding the sensitivity of data is crucial for its proper handling, storage, and transmission. Different sensitivity levels dictate the measures and precautions to be taken.

Confidential::
Data that, if disclosed, could have a severe impact. Examples: private keys, passwords.

Personal::
Data related to individual identity. Examples: public keys, addresses, transactions.

Semi-public::
Data with limited sensitivity. Examples: cryptographic data like nonces or salts.

Public::
Open data with no repercussions if accessed. Examples: signed hashes, public announcements.
...
package "Massa Wallet" {
    package "API" {
        ...
        usecase "Sign Transaction" as walletAPI_signTransaction
...
stationFront_useDApp --> walletAPI_signTransaction
...
[plantuml, format="svg", id="application-mapping-diagram", width="100%", pdfwidth="100%"]
----
@startuml
package "Browser" {
...
Detailed description of the product's functionalities.
Inputs, processes, and outputs for each functionality.

== Non-Functional Requirements:

Performance requirements, security requirements, usability requirements, etc.
Any other requirements that don't relate directly to specific functionalities.
