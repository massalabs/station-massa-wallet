...
On MacOs, the account files are stored in `~/Library/Application\ Support/massa-station-wallet`.
On Ubuntu, the account files are stored in `~/.config/massa-station-wallet`.
On Windows, the account files are stored in `%APPDATA%\massa-station-wallet`.
