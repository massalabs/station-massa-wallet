...
func (o *TradeRollsURL) Build() (*url.URL, error) {
	...
	var _path = "/api/accounts/{nickname}/rolls"

	nickname := o.Nickname
	if nickname != "" {
		_path = strings.Replace(_path, "{nickname}", nickname, -1)
	} else {
		return nil, errors.New("nickname is required on TradeRollsURL")
	}
...
func (o *TradeRollsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on TradeRollsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on TradeRollsURL")
	}
