...
func (o *BackupAccountURL) Build() (*url.URL, error) {
	...
	var _path = "/api/accounts/{nickname}/backup"

	nickname := o.Nickname
	if nickname != "" {
		_path = strings.ReplaceAll(_path, "{nickname}", nickname)
	} else {
		return nil, errors.New("nickname is required on BackupAccountURL")
	}
...
func (o *BackupAccountURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on BackupAccountURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on BackupAccountURL")
	}
