...
func (o *TransferCoinURL) Build() (*url.URL, error) {
	...
	var _path = "/api/accounts/{nickname}/transfer"

	nickname := o.Nickname
	if nickname != "" {
		_path = strings.Replace(_path, "{nickname}", nickname, -1)
	} else {
		return nil, errors.New("nickname is required on TransferCoinURL")
	}
...
func (o *TransferCoinURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on TransferCoinURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on TransferCoinURL")
	}
