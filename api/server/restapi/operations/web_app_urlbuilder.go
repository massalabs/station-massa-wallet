...
func (o *WebAppURL) Build() (*url.URL, error) {
	...
	var _path = "/web-app/{resource}"

	resource := o.Resource
	if resource != "" {
		_path = strings.ReplaceAll(_path, "{resource}", resource)
	} else {
		return nil, errors.New("resource is required on WebAppURL")
	}
...
func (o *WebAppURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on WebAppURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on WebAppURL")
	}
